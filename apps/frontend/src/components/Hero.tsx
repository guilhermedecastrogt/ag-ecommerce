...
export default function Hero() {
  ...
  return (
    <section id="inicio" ref={ref} onMouseMove={onMove} className="relative min-h-[100dvh] overflow-hidden bg-[#001429]">
      ...
      {/* Red diagonal slab — parallax */}
      <div className="absolute top-[-20%] right-[-5%] w-[52%] h-[140%]" style={{ background: "linear-gradient(170deg, #DF040B, #a50308)", clipPath: "polygon(28% 0%, 100% 0%, 100% 100%, 8% 100%)", transform: `translate(${m.x * 15}px,${m.y * 8}px)`, transition: "transform .9s cubic-bezier(.16,1,.3,1)" }} />
      <div className="absolute top-[-20%] right-[-5%] w-[52%] h-[140%] opacity-20" style={{ background: "#600", clipPath: "polygon(29% 0%,100% 0%,100% 100%,9% 100%)", filter: "blur(60px)" }} />

      {/* White wedge */}
      <div className="absolute bottom-0 right-0 w-[38%] h-[44%] bg-white" style={{ clipPath: "polygon(45% 100%,100% 15%,100% 100%)", transform: `translate(${m.x * -8}px,${m.y * -5}px)`, transition: "transform 1.1s cubic-bezier(.16,1,.3,1)" }} />
      ...
      {/* Geo X shapes */}
      <div className="absolute -top-24 -right-24 w-[600px] h-[600px] opacity-[0.03] pointer-events-none" style={{ animation: "heroSpin 70s linear infinite" }}>
      ...
      <div className="absolute -bottom-32 -left-32 w-[350px] h-[350px] opacity-[0.02] pointer-events-none" style={{ animation: "heroSpin 50s linear infinite reverse" }}>
      ...
      {/* Scan line */}
      <div className="absolute left-0 right-0 h-px bg-gradient-to-r from-transparent via-white/10 to-transparent pointer-events-none" style={{ animation: "heroScan 7s ease-in-out infinite" }} />

      <Particles />

      {/* ═══ DIESEL HEART — absolute positioned, bleeds right ════ */}
      <div
        className={`absolute right-[-8%] lg:right-[-4%] xl:right-[2%] top-1/2 transition-all duration-[1.2s] ease-out ${on ? "opacity-100 translate-y-[-50%] scale-100" : "opacity-0 translate-y-[-45%] scale-[0.92]"}`}
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-5 sm:px-8 lg:px-12 min-h-[100dvh] flex items-center">
        <div className="max-w-[560px] xl:max-w-[620px] py-32 lg:py-20">
          ...
          <div className={`flex flex-wrap gap-3 mb-8 transition-all duration-700 ${on ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`} style={{ transitionDelay: "300ms" }}>
            ...
            <a href="#emergencia" className="group btn !px-7 !py-3.5 !rounded-xl !bg-white/[0.06] !border-white/[0.12] text-white hover:!bg-white/[0.12] hover:!border-white/20 backdrop-blur-sm">
            ...
            <a href="https://loja.aguiadiesel.com.br/loja/" target="_blank" rel="noopener noreferrer" className="group btn !bg-transparent !border-0 text-white/30 hover:text-white !px-3">
          ...
          <div className={`flex flex-wrap gap-6 transition-all duration-700 ${on ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`} style={{ transitionDelay: "450ms" }}>
            {STATS.map((s, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-white/[0.06] flex items-center justify-center text-red">
                  <s.icon className="w-4 h-4" />
                </div>
                <div>
                  <span className="font-[var(--font-display)] text-white font-extrabold text-xl tracking-wider leading-none block">
                    {s.pre}{counts[i]}{s.suf}
                  </span>
                  <span className="text-white/30 text-[0.6rem] tracking-wider font-medium uppercase">{s.label}</span>
                </div>
              </div>
            ))}
          </div>
